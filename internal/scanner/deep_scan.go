...
func RunDeepScan(directory, wordlistPath, outputFile string, minScore, maxEvidence int, vtApiKey string, verbose bool, defaultWordlist embed.FS) error {
	...
	fileSummary, err := ScanDirectory(directory, cfg, verbose)
	if err != nil {
		done <- true
		return err
	}
